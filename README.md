...
The same CI job also runs the identical `greeting` scenario once as a full baseline and once as a proof-aware incremental run. It records exact integer before/after pairs for build and test wall time and peak RSS, along with the matching scenario/source/contract/fixture/toolchain/runner identity. That pair is the only basis for a `CLOSED` improvement claim. Utility remains `UNKNOWN` until external user workload evidence exists.
