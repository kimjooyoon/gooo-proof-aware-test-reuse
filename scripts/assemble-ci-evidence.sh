...
for stage in compile build test conformance integration; do
	...
	if ! jq -e --arg stage "$stage" '.stage == $stage and (.wall_ms|type) == "number" and (.wall_ms|floor) == .wall_ms and (.wall_ms >= 0) and (.peak_rss_kib|type) == "number" and (.peak_rss_kib|floor) == .peak_rss_kib and (.peak_rss_kib >= 0) and .exit_code == 0' "$stage_root/$stage.json" >/dev/null; then
		echo "invalid stage metric: $stage" >&2
		jq . "$stage_root/$stage.json" >&2
		exit 1
	fi
done
...
if ! jq -e '.total > 0 and .selected == .total and .failed == 0 and .unknown == 0' "$stage_root/tests.json" >/dev/null; then
	echo "invalid Go test counts" >&2
	cat "$stage_root/tests.json" >&2
	exit 1
fi
